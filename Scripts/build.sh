...
staging="$PWD/dist/.Photo Viewer.staging.app"
rm -rf "$staging"
...
for architecture in $architectures; do
    swift build --disable-sandbox --cache-path "$PWD/.build/cache" --config-path "$PWD/.build/config" \
        ...
        --triple "$architecture-apple-macosx14.0" -c "$configuration" --product PhotoViewer
    ...
    binaries+=("$binary_path/PhotoViewer")
done
lipo -create "${binaries[@]}" -output "$staging/Contents/MacOS/PhotoViewer"
