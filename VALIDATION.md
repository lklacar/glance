...
## Asynchronous loading for network volumes

All 25 core check groups, the window/rendering checks, and the new network-loading controller checks pass. The new suite injects stalled path resolution, cache-version checks, image reads, folder enumeration, and watcher creation into the real window controller. It checks main-run-loop responsiveness, immediate cancellation, retained previous frames, cached pixel reuse, navigation past a stalled read, opening another file during a stalled lookup, ignoring stale completions, closing during pending work, and recovery after a simulated disconnected-volume error. An audit asserts that every injected filesystem operation runs off the main thread.

Cache access itself now performs no filesystem I/O. Version checks run on background workers before and after decoding. Image bytes are read into memory on a worker before ImageIO/AppKit decoding, so the image source cannot defer network reads to display or animation. Recent images are stored as URL strings instead of resolving document bookmarks, and the title uses the filename without requesting a represented-file icon from AppKit. Existing bookmark-based recent entries are not migrated.

The UI shows the pending filename, spinner, and Cancel control; Escape cancels loading. Worker concurrency is bounded, and queued obsolete work is cancelled. A system filesystem call already in progress cannot necessarily be interrupted: cancelling suppresses delivery, while that worker may remain occupied until the OS returns. Two stalled foreground reads can therefore delay later loads without blocking the UI. These are simulated slow-I/O checks, not validation against a live SMB/NFS server or a real network disconnect.

The updated optimized universal app passes metadata, signature, icon, license, and dependency verification and is installed at `/Applications/Glance.app`. A previous-app backup is retained under `.build/backup-before-async-loading`. The installed app was observed displaying an image with navigation and zoom controls enabled. The local ZIP and SHA-256 checksum were refreshed.
